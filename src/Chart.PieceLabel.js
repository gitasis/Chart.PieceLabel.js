...
(function () {
            ...
            if (this.position === 'outside' ||
                ...
                if (this.position === 'outside') {
                    ...
                    if (position.x < view.x) {
                        position.x = view.x - rangeFromCentre - 1.5*offset;
                ...
                if (this.overlap) {
                    ...
                    if (this.segment) {
                        ...
                        var end = {
                            ...
                            y : view.y + (Math.sin(centreAngle) * rangeFromCentre + this.fontSize / text.split('\n').length )
                        };
                        ctx.beginPath();
                        ...
                        if (start.x <= end.x) {
                            position.x = position.x + this.textOffset;
                            end.x = left+ 1.2*offset;
                            if (prev_x <= end.x){
                                ctx.lineTo(end.x, end.y);
                            }
    ...
    PieceLabel.prototype.parseOptions = function (chartInstance) {
        ...
        if (pieceLabel) {
            ...
            this.textOffset = !pieceLabel.is_mobile  ? 20 : 10;
            return true;
        } else {
            return false;
        }
    ...
    PieceLabel.prototype.fillText = function (text, position, fontColor) {
        ...
        if (typeof text === 'object') {
            ...
            var y = position.y - this.fontSize / 2
            for (var i = 0; i < text_arr.length; i++) {
                ctx.fillText(text_arr[i], position.x, y);
                y += this.fontSize;
            }
        }
    };
